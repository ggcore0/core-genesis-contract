...
contract CoreAgent is ICoreAgent, System, IParamSubscriber {
  ...
  function _enableStakeWeight(address delegator) internal {
    ...
    delegatorMap[delegator].isStakeWeight = true;
  ...
  function _delegateCoin(address candidate, address delegator, uint256 amount, bool isTransfer, bool setStakeWeight) internal returns (uint256, bytes32) {
    ...
    if (!setStakeWeight) {
      require(!delegatorMap[delegator].isStakeWeight, 'Already set stake weight');
  ...
  function getStakeWeight(address delegator) external view returns (bool) {
    return delegatorMap[delegator].isStakeWeight || delegatorMap[delegator].candidates.length == 0;
  }
}